...
st.markdown("""
    ...
    [data-testid="stAppViewContainer"] { background-color: #E3F2FD !important; }[data-testid="stSidebar"] { background-color: #BBDEFB !important; border-right: 1px solid #90CAF9; }
...
if not df.empty:
    ...
    with tab4:
        ...
        df_seguro = df_base[df_base["Placa"].str.startswith("SEGURO", na=False)]
        if not df_seguro.empty:
            gasto_total_seguro = df_seguro["Custo de seguro"].sum()
            orc_total_seguro = sum(ORCAMENTOS_SEGURO.get(inst, 0) for inst in inst_ativas)
            perc_seguro = (gasto_total_seguro / orc_total_seguro * 100) if orc_total_seguro > 0 else 0
            ...
            custo_seguro_base = df_seguro.groupby('Base')['Custo de seguro'].sum().reset_index().sort_values('Custo de seguro', ascending=True)
            fig_seg = px.bar(custo_seguro_base, x='Custo de seguro', y='Base', orientation='h', text='Custo de seguro', color='Custo de seguro', color_continuous_scale='Blues')
            fig_seg.update_traces(texttemplate='<b>R$ %{text:,.2f}</b>', textposition='outside', textfont=ESTILO_TEXTO)
            fig_seg.update_layout(height=400, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(showticklabels=False), coloraxis_showscale=False)
            st.plotly_chart(fig_seg, use_container_width=True)
        else:
            st.info("Nenhum lançamento de seguro encontrado.")
