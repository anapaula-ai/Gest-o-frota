...
def load_data():
    try:
        ...
        # CORREÇÃO: Força o formato de data e IGNORA linhas vazias ou inválidas
        df['Mês Referência'] = pd.to_datetime(df['Mês Referência'], errors='coerce', dayfirst=True)
        df = df.dropna(subset=['Mês Referência']) # Isso remove o 'nan' misterioso!
        ...
        # Limpeza Anti-Duplicação de Placas
...
if not df.empty:
    ...
    # CORREÇÃO: Garante que a lista de meses não puxe nomes inválidos (nan)
    ...
    if len(lista_meses) > 0:
        ...
        # CORREÇÃO: Tratamento de segurança extra para achar o número do mês
        df_mes_atual = df_ano[df_ano["Mes_Nome"] == mes_sel]
        mes_num_atual = df_mes_atual["Mes_Num"].iloc[0] if not df_mes_atual.empty else 1
        ...
        st.warning("Nenhum mês com dados válidos encontrado na planilha. Verifique a coluna de data.")
else:
