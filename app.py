...
if not df.empty:
    ...
    with tab1:
        ...
        fig_b_c.update_layout(height=350, separators=',.', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=150, t=0, b=0), xaxis=dict(showticklabels=False, showgrid=False, range=[0, ranking_base_c['Custo de Combustível'].max()*1.3 if not ranking_base_c.empty else 1]), yaxis=dict(title=""))
