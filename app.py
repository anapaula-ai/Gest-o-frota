...
if not df.empty:
    ...
    with tab3:
        ...
        df_comb_acum = df_comb_mes[df_comb_mes["Mes_Num"] <= mes_num_atual]
        orc_c = sum(ORCAMENTOS_COMB.get(i, 0) for i in inst_ativas)
        gasto_c = df_comb_acum["Custo de combustível"].sum()
        perc_c = (gasto_c/orc_c*100) if orc_c>0 else 0
        c_c1, _ = st.columns([1, 3])
        with c_c1: draw_card("EXECUÇÃO COMBUSTÍVEL ANUAL", fmt_br(gasto_c, True), f"{perc_c:.1f}% consumido", progress=perc_c)

    with tab4:
        st.markdown("### 🛡️ Gestão de Seguro")
        df_seg = df_base[df_base["Placa"].str.startswith("SEGURO", na=False)]
        orc_s = sum(ORCAMENTOS_SEGURO.get(i, 0) for i in inst_ativas)
        gasto_s = df_seg["Custo de seguro"].sum()
