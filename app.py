...
st.markdown("""
    ...
    .metric-label { color: #546E7A; font-size: 14px; font-weight: bold; text-transform: uppercase; }
    .metric-value { color: #0277BD; font-size: 26px; font-weight: bold; }
...
def load_data():
    try:
        ...
        else: df['Ano'] = pd.to_numeric(df['Ano'], errors='coerce').fillna(2026).astype(int)
    ...
    with tab1:
        ...
        with g1:
            ...
            fig_km.update_layout(
                ...
                font_color="#01579B",
                margin=dict(r=80), # Espaço extra na direita
                xaxis=dict(range=[0, top10_km['Quilometragem'].max() * 1.15]) # Folga no eixo X
            )
            st.plotly_chart(fig_km, use_container_width=True)

        with g2:
            st.subheader("Maior custo de manutenção")
            ...
            fig_custo.update_layout(
                ...
                plot_bgcolor='rgba(0,0,0,0)', 
                font_color="#01579B",
                margin=dict(r=100), # Espaço extra na direita para R$
                xaxis=dict(range=[0, top10_custo['Custo de manutenção'].max() * 1.25]) # Folga maior para moeda
            )
